  ...
  const result = await renderPage({
    ...
    mainArea: {
      ...
      render: () => {
        ...
        console.log(`    \x1b[33m${tc('instanceManagement.zombie', { count: lastDetection.zombie.length })}\x1b[0m`);
        ...
        if (lastDetection.orphaned.length === 0 && lastDetection.zombie.length === 0 && lastDetection.dead.length === 0 && lastDetection.disconnected.length === 0) {
          ...
          if (lastDetection.zombie.length > 0) {
            console.log(`  \x1b[33m${tc('instanceManagement.foundZombies', { count: lastDetection.zombie.length })}\x1b[0m`);
          }
